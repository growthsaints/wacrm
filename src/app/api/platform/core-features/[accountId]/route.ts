...
import { CORE_FEATURES, type CoreAccessType } from '@/lib/core-features/features'

const ACCESS_TYPES: readonly CoreAccessType[] = [
...
/** GET /api/platform/core-features/[accountId] — one tenant's license row (null = default-on, no row yet). */
...
/**
 * PATCH /api/platform/core-features/[accountId] — create-or-update a
 * tenant's core-feature license. Same field-level audit trail as
 * /api/platform/enterprise/[accountId].
 */
export async function PATCH(
  ...
  try {
    ...
    const limit = checkRateLimit(
      `platform:core-features:${ctx.userId}`,
    ...
    for (const feature of CORE_FEATURES) {
      ...
      if (typeof body[feature] === 'boolean') updates[key] = body[feature]
    }
    if (typeof body.accessType === 'string' && ACCESS_TYPES.includes(body.accessType as CoreAccessType)) {
    ...
    const { data: saved, error: upsertErr } = await admin
      ...
      .single()
    if (upsertErr) {
      console.error('[PATCH /api/platform/core-features/:id] upsert error:', upsertErr)
    ...
    if (auditRows.length > 0) {
      const { error: auditErr } = await admin.from('core_feature_audit_log').insert(auditRows)
      if (auditErr) console.error('[PATCH /api/platform/core-features/:id] audit log insert error:', auditErr)
    }
