...
export default function SmsBroadcastListPage() {
  const router = useRouter();
  const [broadcasts, setBroadcasts] = useState<SmsBroadcastRow[]>([]);
  ...
  const fetchBroadcasts = useCallback(async () => {
    try {
      ...
      setBroadcasts((data ?? []) as SmsBroadcastRow[]);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load SMS broadcasts');
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Bulk SMS sent via your connected SMS Gateway devices.</p>
          </div>
        </div>
        <Button
            ...
            <tbody className="divide-y divide-border">
              {broadcasts.map((b) => (
                <tr
                  ...
                  onClick={() => router.push(`/broadcasts/sms/${b.id}`)}
                >
