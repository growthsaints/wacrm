...
interface HttpSmsRecipientRow {
  ...
  httpsms_config: { label: string | null } | null;
}
...
export default function HttpSmsBroadcastDetailPage() {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const broadcastId = params.id as string;

  const [broadcast, setBroadcast] = useState<HttpSmsBroadcastRow | null>(null);
  const [recipients, setRecipients] = useState<HttpSmsRecipientRow[]>([]);
  const [loading, setLoading] = useState(true);
  const { retryFailedHttpSmsBroadcast, isProcessing } = useHttpSmsBroadcast();

  const fetchData = useCallback(async () => {
    const supabase = createClient();
    const [{ data: b }, { data: r }] = await Promise.all([
      supabase.from('httpsms_broadcasts').select('*').eq('id', broadcastId).maybeSingle(),
      supabase
        .from('httpsms_broadcast_recipients')
        .select('id, status, error_message, sent_at, contact:contacts(name, phone), httpsms_config:httpsms_config(label)')
        .eq('httpsms_broadcast_id', broadcastId)
        .order('created_at', { ascending: true }),
    ]);
    setBroadcast(b as HttpSmsBroadcastRow | null);
    setRecipients((r ?? []) as unknown as HttpSmsRecipientRow[]);
    setLoading(false);
  }, [broadcastId]);
  ...
  async function handleRetryFailed() {
    if (!broadcast) return;
    try {
      await retryFailedHttpSmsBroadcast(broadcast.id);
  ...
  if (!broadcast) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-2">
        <p className="text-sm text-red-400">httpSMS broadcast not found</p>
