// ============================================================
// Resolve the public base URL a request arrived on.
//
// `new URL(request.url).origin` is NOT safe for this on a proxied
// deployment (CloudPanel/nginx, Vercel, Cloudflare, ...) — Next.js
// sees the URL its own process is bound to (e.g. http://localhost:3000
// or an internal port), not the public hostname the browser used. Any
// redirect built from that origin sends the browser to an
// unreachable internal address instead of the real domain — this bit
// the Supabase auth callback redirect once already (see git history)
// after the invite-link flow hit the identical bug first.
//
...
export function getBaseUrl(request: Request, logLabel = "getBaseUrl"): string {
